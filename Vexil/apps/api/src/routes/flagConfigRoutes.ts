...
export default async function flagConfigRoutes(fastify: FastifyInstance) {
    ...
    fastify.get("/:projectId/environments/:environmentId/flags/:flagId", { schema: { tags: ["Flag Config"], summary: "Get flag config", security: s } }, ctrl.getFlagConfig as any);
    fastify.put("/:projectId/environments/:environmentId/flags/:flagId", { preHandler: [adminOrMember], schema: { tags: ["Flag Config"], summary: "Set flag config", security: s, body: { type: "object", required: ["isEnabled"], properties: { isEnabled: { type: "boolean" }, strategyType: { type: "string" }, strategyConfig: { type: "object" }, scheduledAt: { type: "string", nullable: true } } } } }, ctrl.setFlagConfig as any);
}
